#Requires -Version 7
# Builds FlangeDA.IntegrationTests and runs it in accoreconsole.
...
$csproj     = Join-Path $scriptDir "FlangeDA.IntegrationTests.csproj"
$outDir     = Join-Path $scriptDir "bin\$Config\$Tfm"
$testDll    = Join-Path $outDir    "FlangeDA.IntegrationTests.dll"
...
# FlangeDASetupTest writes test data (command context — doc lock held automatically).
