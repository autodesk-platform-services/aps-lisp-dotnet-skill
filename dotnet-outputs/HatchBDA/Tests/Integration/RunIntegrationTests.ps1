#Requires -Version 7
# Builds HatchBDA.IntegrationTests and runs it in accoreconsole.
...
$csproj     = Join-Path $scriptDir "HatchBDA.IntegrationTests.csproj"
$outDir     = Join-Path $scriptDir "bin\$Config\$Tfm"
$testDll    = Join-Path $outDir    "HatchBDA.IntegrationTests.dll"
...
# HatchBDASetupTest writes test data (command context — doc lock held automatically).
