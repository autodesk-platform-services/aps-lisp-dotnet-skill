#Requires -Version 7
# Builds ViewsIO.IntegrationTests and runs it in accoreconsole.
...
$csproj     = Join-Path $scriptDir "ViewsIO.IntegrationTests.csproj"
$outDir     = Join-Path $scriptDir "bin\$Config\$Tfm"
$testDll    = Join-Path $outDir    "ViewsIO.IntegrationTests.dll"
...
# ViewsIOSetupTest writes test data (command context — doc lock held automatically).
